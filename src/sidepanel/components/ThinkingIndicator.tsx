    ...
    <div className="flex flex-col my-2 items-start animate-in fade-in duration-200 w-full max-w-full">
      <div className="flex items-center gap-1.5 mb-1 font-mono text-[9px] text-neutral-500">
        ...
        <span className="flex items-center gap-1 text-emerald-400 font-semibold">
          <Loader2 className="w-3 h-3 animate-spin text-emerald-400" strokeWidth={2} />
          <span>{isExecutingTool ? `EXECUTING [${activeToolName || 'TOOL'}]` : 'THINKING'}</span>
        </span>
      </div>

      <div className="w-full rounded-md p-3 text-xs bg-black border border-neutral-800 text-neutral-200 space-y-2.5 overflow-hidden">
        ...
        <div className="flex items-center gap-2 text-neutral-300 font-mono text-[11px]">
          ...
          <span className="font-medium truncate">
            {statusText || (isExecutingTool ? `Menjalankan tool ${activeToolName}...` : 'Menganalisis halaman...')}
          </span>
        </div>

        {/* Skeleton Loading Indicator */}
        <div className="space-y-1.5 py-1">
          <div className="h-2 bg-neutral-900 rounded animate-pulse w-3/4"></div>
          <div className="h-2 bg-neutral-900/80 rounded animate-pulse w-full"></div>
          <div className="h-2 bg-neutral-900/60 rounded animate-pulse w-5/6"></div>
        </div>

        {/* Thought Process Monospace Box */}
        {thought && (
          <div className="p-2.5 bg-neutral-950 rounded border border-neutral-900 font-mono text-[10px] text-neutral-400 space-y-1 overflow-hidden">
